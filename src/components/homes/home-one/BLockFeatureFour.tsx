               ...
               <div className="col-lg-6 d-flex order-lg-last">
                  <div className="ps-xxl-5 ms-xl-4 pt-100 xl-pt-80 pb-45 w-100 h-100 wow fadeInRight">
                     <div className="title-one mb-60 lg-mb-40">
                        <div className="upper-title">{t('estimateSection.label')}</div>
                        <h3>{t('estimateSection.title')} <span>{t('estimateSection.titleHighlight')}<Image src={titleShape} alt="" className="lazy-img" /></span> {t('estimateSection.titleEnd')}</h3>
                        <p className="fs-24 color-dark">{t('estimateSection.subtitle')}</p>
                     </div>
                     <form onSubmit={(e) => e.preventDefault()} className="me-xl-4">
                        <input type="email" placeholder={t('estimateSection.placeholder')} />
                        <button>{t('estimateSection.button')}</button>
                     </form>
                     <div className="fs-16 mt-10 opacity-75">{t('estimateSection.accurateInfo')} <Link href="/contact" className="fst-italic color-dark text-decoration-underline">{t('estimateSection.contactUs')}</Link></div>
                  </div>
               </div>
                  ...
                  <div className="img-gallery position-relative z-1 w-100 h-100 me-lg-5 wow fadeInLeft">
                     ...
                     <div className="card-one">
                        <div className="text text-center z-1">
                           <h6>{t('estimateSection.yourEstimate')}</h6>
                           <h3>$378,30.00</h3>
                        </div>
